...
import { isInt, isNumber } from "../number";
import { pipe } from "../pipe";
import { OkOf, Result } from "../result";
import * as R from "../result/fp";
...
export namespace nextFloat {
   ...
   */
  export function between(min: number, max: number): Result<number, between.Error> {
    return pipe(
      OkOf({ min, max }),
