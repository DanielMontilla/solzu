 ...
 */
export function peek<V, E>(result: Result<V, E>, f: (ok: V) => void): Result<V, E> {
 ...
 * @returns {Result<V, E>} The original result.
 */
export function peekErr<V, E>(result: Result<V, E>, f: (err: E) => void): Result<V, E> {
 ...
 */
export function or<V, E>(result: Result<V, E>, alternative: V | ((error: E) => V)): V {
  return isOk(result)
    ? result.value
    : isFunction(alternative)
      ? alternative(result.error)
      : alternative;
}
 ...
 */
export function unfold<V, E>(result: Result<V, E>): Result.Unfold<Result<V, E>> {
 ...
 */
export function flatten<V, E>(result: Result<V, E>): Result.Flatten<Result<V, E>> {
...
export function flatmap<FromValue, FromErr, ToValue, ToErr>(
  ...
  return isOk(result) ? (mapper(result.value) as Result<ToValue, FromErr | FromValue>) : result;
}
...
export function check<V, E, FailErr>(
  ...
  return isOk(result) ? (predicate(result.value) ? result : Err(failError)) : result;
}

/**
 * Converts a `Result` into a `Maybe`, returning `Some` if `Ok`, or `None` if `Err`.
 * @template V The type of the `Ok` value.
 * @param {Result<V, any>} result The result to convert.
 * @returns {Maybe<V>} The corresponding `Maybe` value.
 * @see {@link Maybe}
 */
export function toMaybe<V>(result: Result<V, any>): Maybe<V> {
