...
import { Procedure, Operator, Callable } from "../types";
 ...
 */
export function pipe<Input>(input: Input | Procedure<Input>): Input;
...
export async function pipeAsync(input: any, ...operators: Callable[]): Promise<any> {
  let value = isFunction(input) ? input() : isPromise(input) ? await input : input;
