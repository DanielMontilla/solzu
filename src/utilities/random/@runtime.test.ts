...
import { Err, Ok, nextFloat, nextIndex, nextInt, nextKey } from "../..";
...
describe("between", () => {
  ...
  describe("betweenU", () => {
    it("generates a random number within the specified range without validation", () => {
      const num = nextFloat.between.unsafe(1, 10);
      expect(num).toBeGreaterThanOrEqual(1);
      expect(num).toBeLessThanOrEqual(10);
    });
    ...
    it("generates a random number within the specified range with flat distribution", () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const num = nextFloat.between.unsafe(min, max);
    ...
    it("should always return value if min === max (integers)", () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const num = nextFloat.between.unsafe(i, i);
        expect(num).toBe(i);
      }
    });
    ...
    it("should always return value if min === max (floats)", () => {
      ...
      let value = 0;
      for (let i = 0; i < iterations; i++) {
        const num = nextFloat.between.unsafe(value, value);
        expect(num).toBe(value);
        value += icrements;
      }
    });
...
describe("nextInt", () => {
  ...
  describe("betweenU", () => {
    it("returns an integer within the specified range", () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const num = nextInt.between.unsafe(min, max);
        expect(Number.isInteger(num)).toBe(true);
        expect(num).toBeGreaterThanOrEqual(min);
        expect(num).toBeLessThanOrEqual(max);
      }
...
describe("nextIndex", () => {
  ...
  it("returns a valid index for a non-empty array", () => {
    ...
    expect(result).toBeInstanceOf(Ok);
    expect(result.take()).toBeGreaterThanOrEqual(0);
    expect(result.take()).toBeLessThan(array.length);
  });
});
...
describe("nextIndexU", () => {
  it("generates a random index within the array bounds", () => {
    ...
    for (let i = 0; i < iterations; i++) {
      const index = nextIndex.unsafe(array);
...
describe("nextKey", () => {
  describe("@overload for Record", () => {
    it("should return a valid key for a non-empty record", () => {
      ...
      expect(result).toBeInstanceOf(Ok);
      expect(Object.keys(record)).toContain(result.take());
    });
  ...
  describe("@overload for Map", () => {
    it("should return a valid key for a non-empty map", () => {
      ...
      expect(result).toBeInstanceOf(Ok);
      expect([...map.keys()]).toContain(result.take());
    });
  ...
  describe("@overload for Set", () => {
    it("should return a valid element for a non-empty set", () => {
      ...
      expect(result).toBeInstanceOf(Ok);
      expect([...set]).toContain(result.take());
    });
