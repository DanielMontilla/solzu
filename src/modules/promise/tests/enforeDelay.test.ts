...
import { enforceDelay } from "..";

describe.concurrent("enforceDelay [runtime]", it => {
  it("should resolve with the result of the given promise after the specified delay", async () => {
    ...
    const value = await enforceDelay(fn, duration);
    const end = Date.now();
  ...
  it("should reject if the provided promise rejects", async () => {
    ...
    await expect(enforceDelay(fn, duration)).rejects.toThrow(errorMessage);
  });
